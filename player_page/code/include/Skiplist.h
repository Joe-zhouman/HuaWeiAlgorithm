...
namespace simple_dsa {
    ...
    protected:
        bool SkipSearch(ListNodePosi<Quadlist<T> *> &qlist, QlistNodePosi<T> &p, T &k);
    public:
        int Size() const { return this->Empty() ? 0 : this->Last()->data_->Size(); }
        int Level() { return this->Size(); }
        bool Put(T);
    };
 ...
 ******************************************************************************************/
    template<typename T>
    bool Skiplist<T>::SkipSearch(ListNodePosi<Quadlist<T> *> &qlist, QlistNodePosi<T> &p, T &k) {
        while (true) {//在每一层
            while ((p->succ_)!= nullptr && (p->entry_ <= k)) {
                p = p->succ_;//从前向后查找
//直到出现更大的key或溢出至trailer
            }
            p = p->pred_;//此时倒回一步，即可判断是否
            if ((p->pred_ )!= nullptr&& (k== p->entry_))
                return true;//命中
    ...
    template<typename T>
    bool Skiplist<T>::Put(T t) {
        T temp = t;//待插入的词条（将被随机地插入多个副本）
        if (this->Empty()) {
            this->InsertAsFirst(new Quadlist<T>);//插入首个Entry
        }
        ListNodePosi<Quadlist<T> *> qlist = this->First();//从顶层四联表的
        QlistNodePosi<T> p = qlist->data_->First();//首节点出发
        if (this->SkipSearch(qlist, p, t)) {//查找适当的插入位置（不大于关键码k的最后一个节点p）
            while (p->below_)p = p->below_;//若已有雷同词条，则需强制转到塔底
        }
        qlist = this->Last();//以下，紧邻于p的右侧，一座新塔将自底而上逐层生长
        QlistNodePosi<T> b = qlist->data_->InsertAfterAbove(temp, p);//新节点b即新塔基座
        while (rand() & 1) {//经投掷硬币，若确定新塔需要再长高一层，则
            ...
            if (!qlist->data_->Valid(p)) {//若该前驱是header
                if (qlist == this->First())//且当前已是最顶层，则意味着必须
                    this->InsertAsFirst(new Quadlist<T>);//首先创建新的一层，然后
                p = qlist->pred_->data_->First()->pred_; //将p转至上一层Skiplist的header
            } else {//否则，可径自
                p = p->above_;//将p提升至该高度
            }
            qlist = qlist->pred_; //上升一层，并在该层
            b = qlist->data_->InsertAfterAbove(t, p, b);//将新节点插入p之后、b之上
        }
